#!/usr/bin/env bash
# =============================================================================
# PredCache v2 Quick Eval — Fixed: EMA demand + no TASER bypass
# =============================================================================
set -euo pipefail
...
RESULT_DIR="/root/MoE-SD/results/predcache_v2"
...
# 1. SD + PredCache v2 (EMA demand + recency)
echo ">>> [1/2] SD + PredCache v2"
VLLM_PLUGINS=elmm \
...
BRISKMOE_SACR=0 BRISKMOE_ELP=0 BRISKMOE_DIPP=0 BRISKMOE_PREDCACHE=1 \
    run_bench "sd_predcache_v2" "$CPU_OFFLOAD_ELMM" \
    --speculative-config "$SPEC_CONFIG"

# 2. SD + PredCache v2 + DIPP
echo ">>> [2/2] SD + PredCache v2 + DIPP"
VLLM_PLUGINS=elmm \
ELMM_CACHE_GB=$ELMM_CACHE_GB \
ELMM_STALE_REMAP=$ELMM_STALE_REMAP \
BRISKMOE_SACR=0 BRISKMOE_ELP=0 BRISKMOE_DIPP=1 BRISKMOE_PREDCACHE=1 \
    run_bench "sd_predcache_v2_dipp" "$CPU_OFFLOAD_ELMM" \
    --speculative-config "$SPEC_CONFIG"

# Summary
echo ""
echo "================================================================"
echo "[$(timestamp)] PredCache v2 Results"
echo "================================================================"

$PYTHON << 'PYEOF'
import json, os
# Include prior results for comparison
result_dirs = {
    ...
    "sd_predcache_v2+dipp": "/root/MoE-SD/results/predcache_v2/sd_predcache_v2_dipp",
}
