...
> ⚠️ **方向修正（基于 TASER 后瓶颈再分析）**：TASER 消除了 95%+ 的 Phase 3
> 开销后，**Phase 4 kernel compute 重新成为绝对主导瓶颈**。下面的方向基于
> Phase 4 的定量分析重新排序。详见 §13 TASER-后瓶颈分析。

| 优先级 | 方向 | 预期影响 | 瓶颈靶点 |
|--------|------|---------|---------|
| P0 | 更长的 benchmark（1000+ tokens/请求） | 更准确的稳态性能评估 | 测量 |
| P1 | **W8A16 Pool Quantization**（INT8 权重 + BF16 激活） | **HBM 带宽减半 → P4 ~1.93× → 整体 ~48%** | Phase 4 (HBM BW) |
| P2 | CUDA Graph 兼容的 TASER stale path | 消除 kernel launch 开销 (~1-2 ms/step) | Phase 4 (launch) |
| P3 | 2:4 结构化稀疏（Ampere native） | 权重读取再减半 + 计算 2× | Phase 4 (HBM + compute) |
| P4 | CUDA Stream 异步验证 | 验证步 Phase 3 不阻塞前向路径 | Phase 3 (已极小) |

---

## 十三、TASER-后瓶颈再分析：Phase 4 Kernel 的本质
...
Phase 4 的绝对时间没有被 TASER 触及——它是纯 GPU kernel 计算。
现在要优化它，必须理解它是**计算受限还是带宽受限**。
...
| moe_intermediate_size | 768 | MoE FFN 中间维度 |
| w13 per expert | (1536, 2048) | gate+up 融合（2×768=1536） |
| w2 per expert | (2048, 768) | down projection |
| 每 expert 权重 | 9.44 MB (BF16) | w13: 6.29 MB + w2: 3.15 MB |
| Pool slots | 17 | 每层缓存的 expert 数 |
| M (decode) | 4 | K=3 推测解码 → 4 个 token |

**HBM 带宽需求**（17 个 expert 全部被访问的极端情况）：

$$\text{HBM reads/layer} = 17 \times 9.44 \text{ MB} = 160.4 \text{ MB}$$

$$t_{mem} = \frac{160.4 \text{ MB}}{768 \text{ GB/s}} = 0.209 \text{ ms}$$

**计算需求**：

$$\text{FLOPs/layer} = 17 \times 2 \times (4 \times 2048 \times 1536 + 4 \times 768 \times 2048) = 641.7 \text{ MFLOPs}$$

$$t_{compute} = \frac{641.7 \text{ MFLOPs}}{38.7 \text{ TFLOPS}} = 0.0166 \text{ ms}$$

**Roofline 判定**：

| 指标 | 值 | 结论 |
|------|-----|------|
| 算术强度 (AI) | **4.0 FLOPs/byte** | 远低于脊点 |
| A6000 BF16 脊点 | 50.4 FLOPs/byte | |
| $t_{mem} / t_{compute}$ | **12.6×** | **极度带宽受限** |
| 实测 P4 | 0.353 ms/layer | |
| HBM 利用率 | 59.2% | kernel launch + 对齐开销 |

**结论：Phase 4 是教科书级的 memory-bandwidth bound 问题。M=4 的 "skinny GEMM"
场景下，GPU 的计算单元利用率不到 8%，几乎所有时间都在等待 HBM 数据搬运。**

### 13.3 优化方向重新评估

#### ❌ 原 P1 "INT8 Pool Quantization → 2× slots → 更高命中率" 的问题

原始描述将 INT8 的收益定位为"更多 cache slots → 更高命中率"。这在 TASER
之后是**错误的优先级排序**：

1. TASER 的自适应间隔已将验证频率降至 <1%，cache 命中/未命中几乎不影响性能
2. 当前命中率 >85% 已经足够高
3. 即使翻倍到 34 slots → 命中率从 85% 升到 ~93%，但 TASER 跳过了 99%
   的 cache lookup，这 8% 命中率提升对应的性能收益 ≈ 0

**INT8 的真正价值是 HBM 带宽减半**——直接攻击 Phase 4 的带宽瓶颈。

#### ✅ 修正后的 P1：W8A16 Pool Quantization

**机制**：Pool 中 expert 权重以 INT8 存储（每参数 1 字节 + per-channel scale），
FusedMoE kernel 在 Triton 内部反量化为 BF16 后计算。

| 指标 | BF16 (当前) | W8A16 (INT8 权重) |
|------|------------|-------------------|
| 每 expert HBM 读取 | 9.44 MB | ~4.73 MB (含 scale) |
| 每层 HBM 总读取 | 160.4 MB | ~80.4 MB |
| 理论 kernel 时间 | 0.209 ms | 0.105 ms |
| 预测实际 P4 时间 | 0.353 ms | ~0.177 ms |
| 每步节省 (×26 层) | — | ~4.6 ms |

**预期端到端加速**：

$$\frac{9.5 \text{ ms}}{9.5 - 4.6 \text{ ms}} = \frac{9.5}{4.9} = 1.94\times \text{ (对 MoE 步)}$$

考虑到非 MoE 部分（attention、shared expert 等）不受影响，实际端到端预期 **+30~48%**。

**可行性**：
- vLLM 的 `invoke_fused_moe_triton_kernel` 已支持 `use_int8_w8a16=True` 参数
- 当前 Direct Dispatch 中硬编码为 `False`，只需启用并提供 scale 张量
- A6000 (SM 8.6, Ampere) 原生支持 INT8 Tensor Core
- 量化可在 expert 加载时在线完成（CPU→GPU copy 时量化）
- SmoothQuant (ICML'23) 等技术可保证精度损失 <0.5%

**与 "更多 slots" 的兼容**：INT8 同时带来了 2× slots 的副产品，但这是次要收益。

#### ✅ P2：CUDA Graph 兼容的 TASER Stale Path

TASER 的 fast path 是**确定性的**：

```
remapped_ids = remap_table[topk_ids]    # 1 次 gather
moe_align_block_size(...)               # 1 次 C++ op
invoke_fused_moe_triton_kernel(W1)      # 1 次 Triton launch
silu_and_mul(...)                       # 1 次 CUDA kernel
invoke_fused_moe_triton_kernel(W2)      # 1 次 Triton launch
output.sum(dim=1)                       # 1 次 CUDA kernel
```

共 ~6 次 kernel launch × 26 层 = 156 次，每次 ~5-10μs = **0.8-1.6 ms/step**。

如果将 fast path 捕获为 CUDA Graph（每层一个 graph），可将 156 次独立
launch 压缩为 26 次 graph replay，每次 ~2μs = **~0.05 ms/step**。

**预期节省**：0.8-1.6 ms/step → ~0.05 ms/step = **节省 ~1 ms/step (~10%)**

**挑战**：
- `topk_ids` 形状固定但值变化 → 需要 CUDA Graph 的 input capture 机制
- `remap_table` 在 stale path 中不变 → graph 友好
- 验证步需要回退到 eager mode → graph/eager 切换逻辑

#### ✅ P3：2:4 结构化稀疏（Ampere Native Support）

A6000 (SM 8.6) 原生支持 2:4 结构化稀疏（Sparse Tensor Core），理论上：
- 权重 HBM 读取再减半（只存非零值 + metadata）
- 计算吞吐翻倍（Sparse TC 2:4 pattern）

与 W8A16 叠加：9.44 MB → 4.73 MB (INT8) → ~2.4 MB (INT8 + 2:4 sparse) = **4× 压缩**。

**挑战**：
- 需要对 expert 权重做 2:4 稀疏剪枝（可能需要微调或 calibration）
- vLLM 的 FusedMoE Triton kernel 目前不支持 sparse 输入
- 精度风险：MoE expert 本身已经是稀疏激活，进一步稀疏权重可能影响质量
- 实现复杂度高

**相关工作**：
- **SparseGPT** (ICML'23)：无需微调的 LLM 结构化剪枝
- **Wanda** (ICML'24)：基于权重-激活乘积的剪枝，适用于 2:4 pattern
- **SR-STE** (NeurIPS'21)：2:4 稀疏训练的直通估计器

#### ↓ P4：CUDA Stream 异步验证（优先级下调）

TASER 使验证频率降至 ~0.78%（每 128 步一次）。即使验证步的 Phase 3 耗时
9.1 ms，均摊到每步只有 $9.1 \times 0.78\% \approx 0.07$ ms。

**这已经是可忽略的开销**，异步验证的 ROI 极低。

### 13.4 硬件约束提醒

| 特性 | A6000 (SM 8.6) | 说明 |
|------|----------------|------|
| INT8 Tensor Core | ✅ 支持 (309.7 TOPS) | W8A16 / W8A8 均可用 |
| FP8 Tensor Core | ❌ 不支持 | 需要 SM 8.9+ (Ada) 或 SM 9.0 (Hopper) |
| 2:4 Structured Sparsity | ✅ 支持 | Ampere Sparse TC |
| CUDA Graphs | ✅ 支持 | 需处理动态 shape |
| HBM 带宽 | 768 GB/s | GDDR6X，非 HBM2e |
...
现状（14.63 tok/s, 1.93× vs v1）
    │
    ├── P0: 更长 benchmark ────────────────── 准确评估稳态性能
    │
    ├── P1: W8A16 INT8 Pool Quantization ──── 预期 +30~48%
    │       ├ HBM 带宽减半（主要收益）
    │       ├ 2× pool slots（次要收益）
    │       └ vLLM kernel 已支持 use_int8_w8a16
    │
    ├── P2: CUDA Graph (stale path) ───────── 预期 +8~15%
    │       ├ 消除 156 次/step kernel launch
    │       └ 仅对 TASER fast path 生效
    │
    ├── P3: 2:4 Structured Sparsity ────────── 预期 +25~40%（与 P1 叠加）
    │       ├ 需要模型剪枝 calibration
    │       └ 需自定义 Sparse Triton kernel
    │
    └── P4: Async Validation ──────────────── 预期 <1%
            └ TASER 已将验证成本均摊至可忽略
```

**乐观叠加预测**（P1 + P2）：

$$14.63 \times 1.40 \times 1.10 \approx 22.5 \text{ tok/s} \quad (2.97\times \text{ vs v1})$$

---

## 十四、BDE 低秩假设验证实验（S0）—— 否定结论

### 14.1 实验目标

验证 **BDE (Base-Delta Expert Decomposition)** 的核心假设：MoE expert 权重是否具有可利用的公共结构，即 $W_i \approx W_{base} + U_i V_i^T$，其中低秩 delta $U_i V_i^T$ 在 rank = 64 时能保留 ≥ 95% 的 Frobenius 范数能量。
...
| 分析层 | 0, 6, 12, 18, 24, 30, 36, 42, 47（共 9 层，均匀采样） |
| 投影 | gate_proj (768×2048), up_proj (768×2048), down_proj (2048×768) |
| 每层采样 expert 数 | 16（总 128 中取代表性 expert） |
| Base 定义 | $W_{base} = \frac{1}{128}\sum_{i=0}^{127} W_i$（均值） |
| Delta 定义 | $\Delta W_i = W_i - W_{base}$ |
| 分析方法 | SVD: $\Delta W_i = U \Sigma V^T$，测量 $\sum_{j=1}^{r} \sigma_j^2 / \sum_{j=1}^{n} \sigma_j^2$ |
| 评估 rank | 8, 16, 32, 64, 128, 256 |

### 14.3 关键实验结果

#### 能量保留率（全层平均）

| Rank | gate_proj | up_proj | down_proj | **总平均** |
|------|-----------|---------|-----------|-----------|
| 8    | 7.0%      | 4.7%    | 6.0%      | **5.9%**  |
| 16   | 11.4%     | 8.2%    | 9.7%      | **9.8%**  |
| 32   | 18.4%     | 14.3%   | 15.9%     | **16.2%** |
| 64   | 29.0%     | 24.6%   | 26.0%     | **26.6%** |
| 128  | 45.0%     | 41.0%   | 42.0%     | **42.7%** |
| 256  | 67.0%     | 64.3%   | 64.9%     | **65.4%** |

> **判定标准**：rank-64 能量保留 ≥ 95% → BDE 可行。  
> **实测结果**：rank-64 仅保留 **26.6%** → **远未达标** ❌

#### Expert 间相似度分析

| 指标 | 值 | 含义 |
|------|-----|------|
| ‖ΔW‖/‖W_base‖ | **1040%** | Delta 是 base 的 10.4×，expert 极度不相似 |
| Pairwise cosine similarity | **≈ 0.000** | expert 权重几乎正交，无公共结构 |
| σ₁₉/σ₀ | **0.43 ~ 0.82** | 奇异值衰减极慢，频谱平坦 → 满秩 |

#### 奇异值衰减特性

以 Layer 24 down_proj 为代表：

```
σ₀=1.000  σ₁=0.859  σ₃=0.791  σ₇=0.730  σ₁₅=0.662  σ₁₉=0.640
```

理想的低秩矩阵应具有 σ₁₉/σ₀ < 0.01 的快速衰减；实测 σ₁₉/σ₀ = 0.64，衰减极其缓慢。

### 14.4 分析与根因

1. **Fine-grained MoE 的本质特性**：Qwen3-30B-A3B 使用 128 个 fine-grained expert，每个仅 768 中间维度。训练过程中，路由机制（top-8 gating）自然将 expert 推向多样化/正交，以覆盖不同的输入子空间。

2. **与 Dense Model 的本质区别**：Dense transformer 的不同层权重可能共享低秩结构（因为处理的是相同数据流的逐步变换）；而 MoE expert 是**并行的互补函数**，被显式训练为彼此不同。

3. **数学必然性**：128 个 expert 在 768×2048 的空间中，自由度为 128×768×2048 ≈ 2 亿参数。如果 expert 共享一个公共 base，那路由机制就没有存在的必要了 — expert 的价值正在于其差异性。

4. **δ/base 比值为 10.4×** 说明"均值" base 本身几乎不携带有效信息，仅是 128 个近正交向量的叠加噪声，范数远小于任何单个 expert。

### 14.5 结论与路线修正

$$\boxed{\text{BDE 对 Qwen3-30B-A3B 不可行 — expert 权重是满秩的正交矩阵，不存在可利用的低秩公共结构}}$$

**路线回退**：Phase 4 (HBM bandwidth) 瓶颈的第一优先解法回到 **W8A16 INT8 Pool Quantization**：

- ✅ **直接将 HBM 读取带宽减半**，无需假设 expert 间结构
- ✅ vLLM 的 fused_moe Triton kernel 已原生支持 `use_int8_w8a16=True`
- ✅ 可利用 A6000 的 INT8 Tensor Core (309.7 TOPS)
- ✅ 预期 MoE 步骤加速 ~1.93×，端到端加速 ~30-48%

BDE 的探索虽然以否定结论告终，但为后续的方向选择提供了坚实的实证基础 — 排除了一条看似诱人但不适用于 fine-grained MoE 架构的路径。

实验脚本：`scripts/analyze_expert_svd.py`  
原始数据：`results/bde_svd_analysis.json`

---

## 十五、W8A16 INT8 Pool Quantization 实验

### 15.1 实验目标

验证 W8A16 INT8 Pool Quantization 对 ELMM MoE offloading 系统的影响：

1. **Triton FusedMoE 核函数**：INT8 vs BF16 kernel latency
2. **CPU→GPU 拷贝带宽**：INT8 vs BF16 expert loading
3. **综合效果**：考虑缓存命中率提升后的端到端加速

### 15.2 实现方案

**对称逐通道量化 (Symmetric Per-Channel Absmax)**：

$$\text{scale}[n] = \frac{\max(|W[n,:]|)}{127}, \quad W_\text{int8}[n,:] = \text{round}\left(\frac{W[n,:]}{\text{scale}[n]}\right)\text{.clamp}(-128, 127)$$

实现修改清单：
- `adapters/elmm_plugin.py`：`_LayerExpertCache` 添加 `_int8_mode`, `_w13_scale`, `_w2_scale`；`load_expert_int8()` 方法；`install()` 精度选择逻辑；`_direct_dispatch_kernel()` INT8 kernel 调用
- `adapters/vllm_elmm_plugin.py`：添加 `ELMM_INT8_POOL` 环境变量控制

**量化精度验证** (scripts/test_int8_pool.py，4项测试全部通过)：

| 测试 | 指标 | 结果 |
|------|------|------|
| Quantize/Dequantize roundtrip | SQNR | 42.6–42.9 dB ✅ |
| Triton kernel INT8 vs BF16 | Cosine sim | 0.9999 ✅ |
| Triton kernel INT8 vs BF16 | Relative diff | 0.82% ✅ |
| Pool 大小 | 压缩比 | 1.99× (9.44 MB → 4.73 MB) ✅ |

### 15.3 Micro-Benchmark 结果

**Part 1: Triton FusedMoE Kernel Latency (W1+SiLU+W2)**

| Batch Size | BF16 (ms) | INT8 (ms) | Speedup |
|------------|-----------|-----------|---------|
| 1 | 0.278 | 0.262 | 1.061× |
| 4 | 0.379 | 0.409 | 0.925× |
| 8 | 0.300 | 0.261 | 1.149× |
| 16 | 0.279 | 0.264 | 1.058× |
| 32 | 0.282 | 0.268 | 1.054× |

**关键发现**：Kernel 性能基本持平。在 batch=1 (单 token decode) 场景下 INT8 略快 (+6.1%)。矩阵尺寸太小无法充分利用 HBM 带宽 (BF16 利用率仅 30%)，kernel launch 开销占主导。

**Part 2: CPU→GPU Expert Copy (per expert)**

| 模式 | 数据量 | 耗时 | 带宽 |
|------|--------|------|------|
| BF16 | 9.44 MB | 0.386 ms | 24.5 GB/s |
| INT8 | 4.73 MB | 0.233 ms | 20.3 GB/s |

$$\boxed{\text{CPU→GPU 拷贝加速: } 1.66\times}$$

**Part 3: 综合效果分析**

INT8 Pool 的核心价值：
1. **Pool 容量翻倍** → 缓存命中率大幅提升 (17 slots → 34 slots)
2. **Cache miss 恢复速度 1.66×** → 更快的 expert 加载
3. **Kernel 性能中性** → 无显著开销

综合考虑命中率提升后的预期加速：

| 命中率场景 | BF16 命中率→INT8 命中率 | BF16 成本 | INT8 成本 | 加速 |
|-----------|----------------------|----------|----------|------|
| 保守 | 80% → 90% | 0.355 ms | 0.285 ms | **+24.5%** |
| 中等 | 70% → 85% | 0.393 ms | 0.297 ms | **+32.7%** |
| 激进 | 60% → 80% | 0.432 ms | 0.308 ms | **+40.2%** |

### 15.4 结论

$$\boxed{\text{INT8 W8A16 Pool Quantization 预期加速 24–40\%，核心收益来自 2× 更大的 Pool 容量和 1.66× 更快的 Cache Miss 恢复}}$$

- 量化精度验证通过 (SQNR 42.6 dB, cosine 0.9999)
- Kernel 计算开销中性 (batch=1 加速 6.1%)
- 核心收益来自缓存层而非计算层
- 完整 E2E 服务端验证待 GPU 内存释放后执行

实验脚本：`scripts/bench_int8_kernel.py`, `scripts/bench_int8_extended.py`, `scripts/test_int8_pool.py`  
原始数据：`results/int8_kernel_benchmark.json`, `results/int8_extended_benchmark.json`
